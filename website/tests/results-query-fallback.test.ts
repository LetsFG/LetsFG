...
test('pending results page adopts the in-flight prefire before falling back to the server bootstrap route', () => {
  ...
  assert.match(pendingSource, /router\.replace\(`\/results\/\$\{encodeURIComponent\(searchId\)\}\?\$\{nextParams\.toString\(\)\}`\)/)
  assert.match(pendingSource, /router\.replace\(`\/results\?\$\{fallbackParams\.toString\(\)\}`\)/)
})