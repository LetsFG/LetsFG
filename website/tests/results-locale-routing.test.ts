...
test('results routing reuses locale helpers instead of hardcoded English fallbacks', () => {
  ...
  const pendingPageSource = readSource('app/results/pending/page.tsx')
  ...
  assert.match(pendingPageSource, /buildLocaleHomePath\(/)
  ...
  assert.doesNotMatch(pendingPageSource, /<Link href="\/en"/)
...
test('active results home navigation keeps client routing warm while polling', () => {
  const searchPageClientSource = readSource('app/results/[searchId]/SearchPageClient.tsx')
  const resultsLoadingSource = readSource('app/results/loading.tsx')
  const searchLoadingSource = readSource('app/results/[searchId]/loading.tsx')

  assert.match(searchPageClientSource, /const handleNavigateHome = \(event: MouseEvent<HTMLAnchorElement>\) => \{/) 
  assert.match(searchPageClientSource, /event\.preventDefault\(\)/)
  assert.match(searchPageClientSource, /const nextHref = event\.currentTarget\.getAttribute\('href'\) \|\| homeHref/)
  assert.match(searchPageClientSource, /cancelSearch\(searchId\)/)
  assert.match(searchPageClientSource, /router\.prefetch\(homeHref\)/)
  assert.match(searchPageClientSource, /router\.push\(nextHref\)/)
  assert.doesNotMatch(searchPageClientSource, /window\.location\.assign\(homeHref\)/)
  assert.match(resultsLoadingSource, /router\.prefetch\(homeHref\)/)
  assert.match(resultsLoadingSource, /<Link href=\{homeHref\} className="res-topbar-logo-link" aria-label="LetsFG home">/)
  assert.match(searchLoadingSource, /router\.prefetch\(homeHref\)/)
  assert.match(searchLoadingSource, /<Link href=\{homeHref\} className="res-topbar-logo-link" aria-label="LetsFG home">/)
})