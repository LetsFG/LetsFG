...
test('homepage passes the detected origin into the search form and keeps the richer ghost hint scoped to untouched auto-prefills', () => {
  ...
  assert.doesNotMatch(formSource, /navigator\.geolocation\.getCurrentPosition/)
  assert.match(formSource, /fetch\('https:\/\/ipinfo\.io\/json'/)
  assert.match(formSource, /fetch\('https:\/\/ipapi\.co\/json\/'/)
