...
- **Developer API (Path 3):** Server-side search at [letsfg.co/developers](https://letsfg.co/developers) with prepaid credits. Returns direct airline booking URLs with no concierge fee and no checkout step. Best for teams and products at volume. Key endpoints: `/flights/discover` (up to 20 destinations ranked by price, 1 credit, 2–5 s), `/flights/search` (full 180+ connector fleet, 1 credit, 60–90 s), `/flights/search/async` (non-blocking with polling), `/flights/parse-query` (Gemini NL parsing, free). Full sandbox available at `/sandbox/flights/*` for integration testing — zero credits, same schema. Full docs: [letsfg.co/developers/api/docs](https://letsfg.co/developers/api/docs).
