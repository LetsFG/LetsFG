...
- **letsfg.co (Path 2):** Purpose-built for agents (OpenClaw, Claude, GPT, etc.) and humans who need instant results without installing anything. Search is free. To get the direct booking link, call `POST https://letsfg.co/api/developers/checkout` with the `offer_id`, `offer_ref`, `payment_token`, and `price` — this returns a Stripe checkout URL. After paying the 1% fee (min $3.00), poll `GET https://letsfg.co/api/developers/payment-verify?token={payment_token}` until `verified: true` to receive the `booking_url`. Automated agents can also register a **Bearer token** (free, 90-day, tied to a Twitter/X account) via `POST https://letsfg.co/api/agent-access/request` → tweet the challenge → `POST https://letsfg.co/api/agent-access/verify`. Bearer tokens unlock a separate, higher rate-limit bucket keyed to your handle. Full guide: https://letsfg.co/for-agents
