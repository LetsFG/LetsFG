...
def _get_client(api_key: str | None = None, base_url: str | None = None) -> LetsFG:
    ...
    if not bt.api_key:
        _err(
            "Could not connect to LetsFG API for auto-registration.\n"
...
def _handle_auth_error(e: LetsFGError) -> None:
    ...
    env_key = os.environ.get("LETSFG_API_KEY") or os.environ.get(
...
def _format_leg_time(
    leg: dict, pos: str = "dep", include_day_offset: bool = False
) -> str:
...
def _convert_display_price(
    amount: float, from_cur: str, to_cur: str, eur_rates: dict[str, float]
) -> tuple[float, str]:
...
def search(
    ...
    max_stops: Optional[int] = typer.Option(
        None,
        "--max-stops",
        "-s",
        help="Max stopovers (0=direct only, 1, 2). Default: no filter",
    ),
    direct: bool = typer.Option(
        False, "--direct", "-d", help="Direct flights only (shortcut for --max-stops 0)"
    ),
    max_browsers: Optional[int] = typer.Option(
        None,
        "--max-browsers",
        "-b",
        help="Max concurrent browsers (1-32, default: auto-detect from RAM)",
    ),
    mode: Optional[str] = typer.Option(
        None,
        "--mode",
        "-m",
        help="Search mode: 'fast' (OTAs + key airlines, 20-40s) or default (all 200+ connectors)",
    ),
    ...
    def _quiet_unraisable(hook_args):
        try:
            if (
                hook_args.exc_type is ValueError
                and "pipe" in str(hook_args.exc_value).lower()
            ):
                return
    ...
    trip_label = (
        f"{origin} ↔ {destination}" if return_date else f"{origin} → {destination}"
    )
    ...
    if HAS_RICH:
        ...
        for i, o in enumerate(offers, 1):
            ...
            price, cur = _convert_display_price(
                raw_price, raw_currency, target_currency, eur_rates
            )
            row = [
                str(i),
                f"{cur} {price:.2f}",
        ...
        for i, o in enumerate(offers, 1):
            ...
            if ob_url and ib_url:
                ...
                print(
                    f"  {i:3d}. {cur} {price:.2f} {airlines}{id_str} — no booking URL"
                )
    else:
        for i, o in enumerate(offers, 1):
            raw_price = o.get("price", 0)
            ...
            id_str = f"  [{offer_id}]" if offer_id else ""
            print(
                f"  {i:3d}. {cur} {price:.2f}  {airlines}  {_route_str(ob)} {dep}→{arr}{ret}{id_str}"
            )
            if ob_url and ib_url:
                print(f"       Outbound: {ob_url}")
                print(f"       Return:   {ib_url}")
...
def unlock(
    ...
    api_key: Optional[str] = typer.Option(
        None, "--api-key", "-k", envvar="LETSFG_API_KEY"
    ),
    base_url: Optional[str] = typer.Option(
        None, "--base-url", envvar="LETSFG_BASE_URL"
    ),
):
    """Unlock a flight offer — confirms price, reserves for 30 minutes."""
    ...
    if result.is_unlocked:
        print(f"\n  ✓ Offer unlocked!")
        print(
            f"    Confirmed price: {result.confirmed_currency} {result.confirmed_price:.2f}"
        )
...
def book(
    ...
    passenger: list[str] = typer.Option(
        ...,
        "--passenger",
        "-p",
        help='JSON passenger object: \'{"id":"pas_xxx","given_name":"John","family_name":"Doe","born_on":"1990-01-15","gender":"m","title":"mr"}\'',
    ),
    ...
    if result.is_confirmed:
        ...
        print(
            f"    Service fee: {result.currency} {result.service_fee:.2f} ({result.service_fee_percentage}%)"
        )
...
def register(
    ...
    print(f"\n    Ready to go! Set your API key:")
    print(f"    export LETSFG_API_KEY={new_key}\n")
...
def recover(
    ...
    code: str = typer.Option(
        "", "--code", "-c", help="6-digit recovery code (if you have one)"
    ),
    output_json: bool = typer.Option(False, "--json", "-j", help="Output raw JSON"),
    base_url: Optional[str] = typer.Option(
        None, "--base-url", envvar="LETSFG_BASE_URL"
...
def me(
    ...
    print(
        f"  Access:  {'✓ Granted (search, unlock, book)' if access else '✗ Not granted'}"
    )
...
def system_info_cmd(
    ...
    print(
        f"  RAM total:    {ram_total:.1f} GB"
        if ram_total
        else "  RAM total:    unknown"
    )
    print(
        f"  RAM available: {ram_avail:.1f} GB"
        if ram_avail
