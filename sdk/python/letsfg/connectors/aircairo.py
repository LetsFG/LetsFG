...
class AirCairoConnectorClient:
    ...
    async def _extract_from_dom(
        ...
        for fd in flight_data:
            ...
            dep_dt = datetime.combine(date.fromisoformat(date_str), datetime.min.time())
            arr_dt = dep_dt
            dur_sec = 0
    ...
    def _parse_api_data(
        ...
        for flight in flights:
            ...
            dep_time = str(flight.get("departureTime") or flight.get("departure") or "")
            arr_time = str(flight.get("arrivalTime") or flight.get("arrival") or "")
    ...
    def _parse_air_bounds_payload(
        ...
        for group in groups:
            ...
            package_price_note = self._describe_group_package_prices(air_bounds, currency_dict)
    ...
    def _describe_group_package_prices(cls, air_bounds: list, currency_dict: dict) -> str:
        ...
        for air_bound in air_bounds if isinstance(air_bounds, list) else []:
            ...
            raw_amount, currency = cls._extract_total_price(air_bound)
            price = cls._normalize_price(raw_amount, currency, currency_dict)
            if price <= 0:
                continue

            seen_families.add(fare_family)
            package_prices.append((fare_family, currency or "USD", price))
    ...
    @staticmethod
    def _parse_dt(s: str, fallback_date: str) -> datetime | None:
        s = (s or "").strip()
        if not s:
            return None
    ...
    def _combine_rt(
        ...
        for o in ob[:15]:
            for i in ib[:10]:
                ...
                merged_conditions, merged_bags = AirCairoConnectorClient._merge_round_trip_metadata(o, i)
