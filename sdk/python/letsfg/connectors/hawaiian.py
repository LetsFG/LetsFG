"""
Hawaiian Airlines direct connector — Playwright + flightresults API.

Hawaiian Airlines (IATA: HA) is a major US carrier based in Honolulu (HNL).
Operates inter-island Hawaii flights, mainland-Hawaii routes, and Asia/Pacific
services. Merged into Alaska Airlines Group — shares the same booking
infrastructure and /search/api/flightresults endpoint.

Strategy:
  1. Launch headed Playwright Chrome → load hawaiianairlines.com to establish
     session cookies (no WAF/CAPTCHA, same SvelteKit SPA as alaskaair.com).
  2. Use in-browser fetch() to POST /search/api/flightresults with IATA codes
     and date. Returns full itineraries with segments, fare classes, pricing,
...
async def _get_browser():
    ...
    async with lock:
        ...
        try:
            ...
            _browser = await pw.chromium.launch(**launch_kw)
        logger.info("Hawaiian: headed Chrome launched")
        return _browser
...
class HawaiianConnectorClient:
    """Hawaiian Airlines connector — Playwright + flightresults API."""
    ...
    async def search_flights(self, req: FlightSearchRequest) -> FlightSearchResponse:
        ...
        try:
            ...
            logger.info(
                "Hawaiian: %s→%s on %s — %d offers in %.1fs",
    ...
    async def _search_via_browser(self, req: FlightSearchRequest) -> list[FlightOffer]:
        ...
        try:
            ...
            logger.info("Hawaiian: loading site for session cookies")
            await page.goto(
            ...
            try:
                ...
                # Homepage can be slow; domcontentloaded is enough for fetch
    ...
    def _parse_rows(
        ...
        for row in rows:
            ...
            for seg_data in segments_data:
                ...
                carrier_code = pub.get("carrierCode", "HA")
                carrier_name = pub.get("carrierFullName", "Hawaiian Airlines")
            ...
            offer_id = f"ha_{hashlib.md5(key.encode()).hexdigest()[:12]}"
